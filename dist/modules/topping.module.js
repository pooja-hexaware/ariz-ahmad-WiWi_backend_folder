"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.toppingModule = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const topping_controller_1 = require("../controllers/topping.controller");
const topping_service_1 = require("../services/topping.service");
const topping_repo_1 = require("../repository/topping.repo");
const topping_schema_1 = require("../schemas/topping.schema");
let toppingModule = class toppingModule {
};
toppingModule = __decorate([
    (0, common_1.Module)({
        imports: [
            mongoose_1.MongooseModule.forFeature([{ name: topping_schema_1.topping.name, schema: topping_schema_1.toppingSchema }])
        ],
        controllers: [topping_controller_1.toppingController],
        providers: [topping_service_1.toppingService, topping_repo_1.toppingRepo],
        exports: [topping_service_1.toppingService, topping_repo_1.toppingRepo]
    })
], toppingModule);
exports.toppingModule = toppingModule;
//# sourceMappingURL=topping.module.js.map